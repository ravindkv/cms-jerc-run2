...
auto RunWqqm::Run(std::shared_ptr<SkimTree>& skimT, PickEvent *pickEvent, PickObject *pickObject, ScaleEvent* scaleEvent, ScaleObject *scaleObject, TFile *fout) -> int{
    ...
    for (Long64_t jentry = 0; jentry < nentries; ++jentry) {
        ...
        pickObject->pickMuons(*skimT);
        std::vector<int> pickedMuons = pickObject->getPickedMuons();
        if (pickedMuons.size() !=1) continue;
        TLorentzVector p4Lep;
        int muInd = pickedMuons[0];
        p4Lep.SetPtEtaPhiM(skimT->Muon_pt[muInd], skimT->Muon_eta[muInd],
                            skimT->Muon_phi[muInd],skimT->Muon_mass[muInd]);
