...
void Helper::printCutflow(const TH1D* hist){
    ...
    std::cout << std::setw(coutTabWidth20) << "CUT" << std::setw(coutTabWidth10) << "ENTRIES" << std::setw(coutTabWidth15) << "REDUCED (%)" << '\n';
    ...
    for (int i = 2; i <= nBins; ++i) {
        ...
        double change = (previous != 0) ? ((previous - current) / previous) * hundred : 0.0;
