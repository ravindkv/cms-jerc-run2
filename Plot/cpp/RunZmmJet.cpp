...
int RunZmmJet::Run(nlohmann::json inputJson, std::string eosPlotDir, Slide &channelSlide, Slide &allChannelSlide){
    channelSlide.startDocument("JME-21-001: L3Residual from ZmmJet channel");
    addZmmJetSlides(channelSlide);

    allChannelSlide.addCenteredTextSlide("Next we show results from ZmmJet channel");

    std::string channel = "ZmmJet";
    ...
    if(isPlotEras1D){
        ...
        for (const auto & config: figConfigVecEras1D){
            ...
            allChannelSlide.addPlotSlide(plotsForSlide, dir+"/"+name+" from ZmmJet channel for different years");
        }//config
    }//isPlot1D
    ...
    if(isPlotEraXY){
        ...
        for (const auto & config: figConfigVecEraXY){
            ...
            for (auto &year: years){
                ...
                channelSlide.addPlotSlide(plotsForSlide, dir+"/"+name+" for "+ year);
                allChannelSlide.addPlotSlide(plotsForSlide, dir+"/"+name+" from ZmmJet channel for "+ year);
            }//year
        }//config
    }//isPlotXY
...
void RunZmmJet::addZmmJetSlides(Slide & channelSlide){
  //----------------------------
  // Samples
  //----------------------------
