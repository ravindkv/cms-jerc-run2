...
void HistRef::InitializeHistograms(TDirectory *origDir, const std::string& directoryName, const VarBin& varBin)
{
    ...
    std::vector<double> rangePt = varBin.getRangePt();
    int ptN   = rangePt.at(0);
    int ptMin = rangePt.at(1);
    int ptMax = rangePt.at(2);
    ...
    hist_.h1EventInRefPt = std::make_unique<TH1D>("h1EventInRefPt", "", ptN, ptMin, ptMax);
    hist_.h1EventInRefMass = std::make_unique<TH1D>("h1EventInRefMass", "", nMass, binsMass.data());
    hist_.h1EventInRefPtForRefBarrel = std::make_unique<TH1D>("h1EventInRefPtForRefBarrel", "",ptN, ptMin, ptMax);
    hist_.h1EventInGenRefPtForGenRefBarrel = std::make_unique<TH1D>("h1EventInGenRefPtForGenRefBarrel", "", ptN, ptMin, ptMax);
