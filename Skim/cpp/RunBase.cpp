...
void RunBase::runEventLoop(NanoTree* nanoT, TFile* fout) {
...
int RunBase::Run(std::shared_ptr<NanoTree>& nanoT, TFile* fout) {
    ...
    if (globalFlags_.is2016Pre || globalFlags_.is2016Post)
        ...
        throw std::runtime_error("Unknown run period in RunBase::Run");

    // Create a ReadConfig instance.
    ReadConfig readConfig("config/RunBase.json", getConfigPath());
