...
template<typename T>
void LoadEraXY<T>::setVarBins(const std::vector<double>& varBins) {
  varBins_ = varBins;
}
template<typename T>
void LoadEraXY<T>::setVarName(const std::string & varName) {
  varName_ = varName;
}

template<typename T>
void LoadEraXY<T>::setVarIsOnXaxis(const bool& varIsOnXaxis) {
...
template<typename T>
void LoadEraXY<T>::drawHists(TDRStyle &tdrS, const std::vector<TH1D*>& hists) {
  if (hists.empty()) {
