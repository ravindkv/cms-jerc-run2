...
int RunWqqe::Run(nlohmann::json inputJson, std::string eosPlotDir, Slide &channelSlide, Slide &allChannelSlide){
    channelSlide.startDocument("JME-21-001: L3Residual from Wqqe channel");
    //addWqqeSlides(channelSlide);

    allChannelSlide.addCenteredTextSlide("Next we show results from Wqqe channel");

    std::string channel = "Wqqe";
    std::vector<std::string> years;
    for (const auto& element : inputJson[channel].items()) {
        years.push_back(element.key());
    }

    ReadConfig readConfig("input/config/json/FigConfigWqqe.json");
    ...
    if(isPlotEras1D){
        ...
        for (const auto & config: figConfigVecEras1D){
            ...
            allChannelSlide.addPlotSlide(plotsForSlide, dir+"/"+name+" from Wqqe channel for different years");
        }//config
    }//isPlot1D
    ...
    if(isPlotEraXYs){
        ...
        for (const auto & config: figConfigVecEraXYs){
            ...
            for (auto &year: years){
                ...
                channelSlide.addPlotSlide(plotsForSlide, dir+"/"+name+" for "+ year);
                allChannelSlide.addPlotSlide(plotsForSlide, dir+"/"+name+" from Wqqe channel for "+ year);
            }//year
        }//config
    }//isPlotXY
...
void RunWqqe::addWqqeSlides(Slide & channelSlide){
  //----------------------------
  // Samples
  //----------------------------
