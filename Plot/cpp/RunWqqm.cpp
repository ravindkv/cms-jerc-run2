...
int RunWqqm::Run(nlohmann::json inputJson, std::string eosPlotDir, Slide &channelSlide, Slide &allChannelSlide){
    channelSlide.startDocument("JME-21-001: L3Residual from Wqqm channel");
    //addWqqmSlides(channelSlide);

    allChannelSlide.addCenteredTextSlide("Next we show results from Wqqm channel");

    std::string channel = "Wqqm";
    std::vector<std::string> years;
    for (const auto& element : inputJson[channel].items()) {
        years.push_back(element.key());
    }

    ReadConfig readConfig("input/config/json/FigConfigWqqm.json");
    ...
    if(isPlotEras1D){
        ...
        for (const auto & config: figConfigVecEras1D){
            ...
            allChannelSlide.addPlotSlide(plotsForSlide, dir+"/"+name+" from Wqqm channel for different years");
        }//config
    }//isPlot1D
    ...
    if(isPlotEraXYs){
        ...
        for (const auto & config: figConfigVecEraXYs){
            ...
            for (auto &year: years){
                ...
                channelSlide.addPlotSlide(plotsForSlide, dir+"/"+name+" for "+ year);
                allChannelSlide.addPlotSlide(plotsForSlide, dir+"/"+name+" from Wqqm channel for "+ year);
            }//year
        }//config
    }//isPlotXY
...
void RunWqqm::addWqqmSlides(Slide & channelSlide){
  //----------------------------
  // Samples
  //----------------------------
